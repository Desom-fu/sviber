...
Below the basic information, there are some basic control options of music playing,
all of which are checkboxes:

- Lock visible range: when checked, the visible range can only be changed by interacting with the timeline scroll bar or the keyboard,
...
- SE: global toggle of note SE when playing music. SE of notes in inactive channels are not played in any case. This is true by default.
- Seek back after playing: when checked, after the music playing stops,
  the current time is changed back to when the music started playing. This is false by default.
- Metronome: when checked, play a synthesized percussion sound at every beat.
...
When no events are selected, display a text saying that no events are selected.
...
is equal to the horizontal visual distance per unit time in the scroll view
...
- SE volume: control note SE and metronome volume, max and default is 1.
- Music volume: control music volume, max and default is 1.
...
with keyboard shortcut <kbd>Ctrl</kbd>+<kbd>Alt</kbd>+<kbd>M</kbd>.
