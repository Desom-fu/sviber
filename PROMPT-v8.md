...
When the music is [playing](#play-pause), the current time change accordingly.
...
When the music is playing, if the current time is not in the visible range or smaller than the center of the visible range,
...
The user can also use <kbd>PageUp</kbd> (to shift the visible range by positive amount)
and <kbd>PageDown</kbd> (to shift the visible range by negative amount) to do the same operation.
...
- Seek backward by 10 s (<kbd>Ctrl</kbd>+</kbd>,</kbd>)
- (separator)
...
#### Seek forward by 10 s, Seek backward by 10 s
