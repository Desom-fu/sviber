...
stdenv.mkDerivation {
  ...
  # and wraps system `nwjs`. Host-Node native rebuilds (sharp, gl, canvas) happen during
  # `npm ci`; desktop builds keep that ABI because rendering runs in a bundled Node worker.
  env = {
