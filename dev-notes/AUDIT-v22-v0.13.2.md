...
- 时间轴总览滚动条空白轨道点击现在按可见范围长度翻页；Ctrl 点击或拖动仍按指针位置吸附当前时间。
- 主编辑区 HUD 底部进度条注册低优先级命中区域，支持点击和拖动寻拍，事件与吸附器命中优先。
- 中英文帮助文档和功能命名单元测试同步覆盖上述行为。
...
- 按功能命名的审计测试和本次交互回归全部通过；矩阵检查确认 4591 个连续行号和 65 条关闭记录。
- `npm run build`: 通过，生成 `build/sviber-0.13.3.nw` 和本地 NW.js 构建目录。
- SSC 专项回归：4 项通过，覆盖事件自动保存/恢复、导入 dirty 状态、媒体异步加载后的撤销、旧快照媒体兼容。
- 受影响功能专项测试：34 项通过；按功能命名的审计测试全部通过。
