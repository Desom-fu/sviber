...
|           |                                    | Inspection panel /    |
|           |                                    | Channels panel /      |
|   Scroll  |                                    | Snappees panel        |
|    view   |         Main editor field          |                       |
|           |                                    |-----------------------|
|           |                                    |                       |
|           |                                    | History panel         |
|           |                                    |                       |
...
- Metronome (`metronome.svg`): when checked, play a synthesized percussion sound at every beat.
...
- Read-only (`read-only.svg`): when checked, most functions of the editor are disabled.
  ...
  items in the "Music" menu, and manipulating comments.
...
and shares the place with the channels panel and the snappees panel.
The user can click the tabs above the panel to switch between the three panels.
...
The channels panel is below the status panel and shares the place with the inspection panel and the snappees panel.
The user can switch among the three panels.
...
The snappees panel is below the status panel and shares the place with the inspection panel and the channels panel.
The user can switch between the three panels.
...
The placement of events in the scroll view is determined by their time and their x coordinate.
...
The current time is always put at the same height near the bottom of the scroll view
...
Event icons for `bgNote` events are always below event icons of other types.
For other cases, event icons of later times is above event icons of earlier times.
...
At the left of the scroll view near every integer beat line,
...
If there are currently selected events (except background patterns) and the event creation mode is not active,
...
If there are not selected events (or if the only selected events are background patterns)
or if the event creation mode is already active, then enter or continue event creation mode.
...
If the mouse is near (the distance threshold is 6.125 in the chart coordinate system)
...
If no events are selected by a snappee is selected,
...
You need to devise a JavaScript API and a Ruby API that codes in macros can use to modify sviber's state.
They must be thorough enough to read and write every aspect of current chart data,
including events, snappees, channels, the current time, etc.
You also need to balance between readability, convenience, and rememberability.
For example, the function to create a `tap` event can be called `tap` and aliased as `t` for convenience.
You can find inspiration from sscharter.
...
dragging the times or end times of events in the timeline, etc.).
...
If there are no auto-saves on app startup, try to open the last project/chart that the user opened previously.
