...
On Chromium, after an audio context plays nothing but silence for several minutes,
its `currentTime` will freeze for a little while when it starts playing something again.
This is unacceptable for a high-precision audio application like sviber.
You can use this code snippet to keep the audio context active:
...
There should be no gap between these panels.
The HTML element to handle the mouse events for resizing should be transparent
so that the panels do not appear with gaps.
...
Switching the chart immediately modifies the project's active chart
and writes the modification to local filesystem.
...
Clicking or dragging on the waveform sets the current time at the mouse position.
The current time is not snapped to subdivisions while dragging on the waveform,
but it is snapped after the mouse releases if the music is not playing.
All operations that requires a snapped current time
(e.g., adding events, adding BPM change, adding bar line,
and pasting) are disabled while dragging.

When the user clicks on the waveform while holding <kbd>Shift</kbd>,
if the place where the user clicks is not close to an A-B loop mark,
this operation creates a new A-B loop mark.
If there are currently two A-B loop marks before the mouse down,
the A-B loop marks are cleared.
Then, an A-B loop mark is created where the mouse button is down, snapped to subdivisions.
If the mouse continues to be dragged and to become close to another subdivision,
this operation additionally creates or moves another A-B loop mark.
As the mouse moves, the position of the other A-B loop mark (not the one created by the mouse down)
is changed to where the mouse is, snapped to subdivisions.
If the mouse drags out of the visible range, smoothly shift the visible range to chase the mouse.

When the user clicks and drags on the waveform while holding <kbd>Shift</kbd>,
if the place where the user clicks is close to an A-B loop mark,
this operation moves an A-B loop mark.
The mark is moved to where the mouse is while the user drags with the mouse, snapped to subdivisions.
The mark can move past the other A-B loop mark.
If the mouse drags out of the visible range, smoothly shift the visible range to chase the mouse.
When the mouse releases, if the new position of the mark is the same as the other mark,
only one mark remains after this operation.
...
If there is one or more hidden channels between them, the line should be colored in bright gray and thickened.
If additionally the current channel is hidden between them, the line should be colored in bright yellow.
If there are hidden channels just above the uppermost shown channel at the current scrolling position of the channels,
the bright line is drawn between it and the waveform.
If there are hidden channels just below the lowermost shown channel at the current scrolling position of the channels,
the horizontal bright line is drawn between it and the scroll bar.
...
The colors of the icons must be very distinguishable from that of selected events (red or magenta).
For perdurant events, show a tail to indicate the duration.
...
If the mouse drags out of the visible range, smoothly shift the visible range to chase the mouse.
If the mouse drags out of the three shown channels, scroll through the channels to chase the mouse.
...
(magenta tint for selected locked events).
...
If there are selected events in a hidden channel
and the horizontal bright line indicating that hidden channel is currently visible,
make small segments of that line red or magenta
according to the time of the selected events in that channel.
...
If the user clicks and drags in the channels while holding <kbd>Alt</kbd>+<kbd>Shift</kbd>,
move all selected events just as how you should move them if the user is dragging them without holding <kbd>Alt</kbd>+<kbd>Shift</kbd>.
The mouse cannot do anything else in the channels if dragging while holding <kbd>Alt</kbd>+<kbd>Shift</kbd>,
so this allows the user to drag events without having to aim the mouse at one selected events,
having to avoid selecting other events,
or having to avoid touching end time handles.
If how the events are moved depends on which specific event the user is interacting with,
handle the movement as if the user is interacting on the event that, when the mouse button was pressed down,
was closest to the mouse.
...
The coloring is relative throughout the chart, with the place with largest density being colored with `#7f1f1f`,
and the place with lowest density being colored with `#1f1f1f`.
...
the visible range shrinks/zooms in (if scrolling up) or enlarges/zooms out (if scrolling down).
If the current time is within the visible range,
make the zooming keep the visual position of the current time unchanged;
otherwise, make the zooming keep the center of the visible range unchanged.
If the enlargement will make the visible range exceed the bounds set by the music,
shift it so that the visible range is still within bounds.
...
When the user clicks and possibly drags on the scroll bar
while holding <kbd>Ctrl</kbd>, the current time is set according to where the mouse is, snapped to subdivisions.
...
- SE (`se.svg`): global toggle of note SE (not including bg note SE) when playing music. SE of notes in inactive channels are not played in any case. This is true by default.
- Bg note SE (`bg-note-se.svg`): toggle of bg note SE when playing music. SE of bg notes in inactive channels are not played in any case. This is false by default.
...
- Show HUD (`show-hud.svg`): when checked, show HUDs in the main editor field (top left HUD, top center HUD, top right HUD, pause button, and progress bar). Checked by default.
- Show chart boundary (`show-chart-boundary.svg`): when checked, show the chart boundary rect in the main editor field. Checked by default.
...
If there are selected events, the status panel shows how many events are selected.
...
with the event stacked at the bottom being drawn at the top.
The connection of double lines (i.e., dash lines connecting simultaneous taps) follow the same order:
if three simultaneous taps, 1, 2, and 3, have the drawing order 1 < 2 < 3,
then a double line connects 1 and 2, and a double line connects 2 and 3,
but there is no double line connecting 1 and 3.
...
(this rule does not apply to `group` events but applies to other types of events,
including those contained directly or indirectly in selected `group` events).
...
When there is only one movable event
and possibly unmovable events in the selection,
...
and all selected movable events are attached to the same rectangular mesh or parametric mesh,
the user can move them together by dragging with mouse.
The changes in $i$ and $j$ for the snap points of all the attached events are the same.
...
the user can move them together by dragging with mouse.
Possible movements are rotating around the center
and moving inward and outward in the radial direction.
The events are still attached to the mesh when being moved.
...
The mouse cannot do anything else in the main editor field if holding <kbd>Shift</kbd>,
so this allows the user to drag events without having to aim the mouse at one selected events,
having to avoid selecting other events,
or having to avoid touching handles of events, tip points, or snappees.
If how the events are moved depends on which specific event the user is interacting with
...
For this mechanics, holding <kbd>Alt</kbd>+<kbd>Shift</kbd> is equivalent to just holding <kbd>Shift</kbd>.
...
The handles (and thus their hit boxs) enlarge when the user holds <kbd>Ctrl</kbd>+<kbd>Alt</kbd>.
...
and show a broken line connecting the control points.
...
and the higher-order control points are connected to their respective first-order control points
(the control points that the pen curve is supposed to pass through) by line segments.
The handle shapes are different for first-order control points and higher-order control points:
the former uses square shape handles, and the latter uses circle shape handles.
...
When the user hits <kbd>Enter</kbd> while focused on an input field in the inspection panel,
...
Every channel also has a button to hide and show it
(use icon `show-channel.svg` for showing and `hide-channel.svg` for hiding).
Hidden channels are not shown but collapsed in the timeline,
but whether a channel is hidden does not affect how events in them appear in the scroll view and the main editor field.
...
Clicking the expansion button again makes the second line disappear.
...
The layout of the 6 elements must arrange them in 3 columns in the conventional form of the matrix.
...
It shares the same space with the checks panel, and the user can switch between them by clicking on tabs.
...
All operations that requires a snapped current time (e.g., adding events, adding BPM change, adding bar line,
and pasting) are disabled while dragging.
...
The checks panel is to the left of the main editor field.
It shares the same space with the scroll view,
and the user can click on tabs to switch between them.

The checks panel list all violations to [chart checks](#chart-checks).
It is updated on live to show all violations at real time as the user edits the chart.
Each item contains texts about the type of check and, if applicable, the time in the music when the violation happens.
The items are sorted by their times, and those that do not have a time are sorted at the top.
Clicking on an item navigates to where the user can fix the violation,
and it can be an event or a popup form.
Hovering over an item shows details about this type of violation in the tooltip bar.
Double clicking an item shows the popup form for [setting checks](#checks),
with the checkbox for disabling the corresponding type of check focused.

The clickable tab of the checks panel also shows the number of violations in red font if it is not zero.
The number updates at real time.
...
The circular arc curve has two parameters for the coordinates of the center,
two coordinates for the start and end angles,
one parameter $m$ for the number of segments,
and two boolean parameters for whether the arc is clockwise or anticlockwise
...
In the popup form for editing parameters of a pen curve,
also show the SVG path data of the curve,
and add a button to copy the SVG path data.
Also add a button to import SVG path data from clipboard.
...
If there is a currently open chart but not a currently open project,
prompt to ask whether the user wants to add the chart to the new project.
If the user does not want to add the chart to the new project and there are unsaved changes,
prompt to ask whether the user wants to save before closing the chart.
If the user does not want to add the chart to the new project,
open a popup form that is the same as that for [creating a new chart](#new-chart) to create the first chart for the project.

#### New chart...

This action creates a new chart.
If a project is currently open, the new chart is added to the open project,
and the music and image of the previously open chart in the project is automatically used for the new chart.
...
This listener only runs if the user edits the difficulty name,
but it does not run before the user edits anything.
...
Opening a chart or importing a chart does not set the value for the next default charter.
The difficulty name is by default "Master", and the difficulty color is by default the preset color for master;
but if that difficulty already exists in the project, the difficulty name is by default "Special",
and the difficulty color is by default the preset color for special.
...
A directory containing `project.sviber` (higher priority) or `sviber-project.json`
is an acceptable project folder.
If the project folder contains the latter but does not contain the former,
rename the latter to the former later when saving chart/project later.
...
Prompt to save if there is currently unsaved changes.
If sviber is in NW.js app and a project is open,
prompt to ask whether the user wants to add the chart (and its referenced music and image) to the project.
Adding the chart to the project modifies the chart list and the active chart of the project,
and the change is immediately written to the filesystem
(including copying files to the project and modifying `project.sviber`)

#### Open recent...

Show a popup form to choose a recently opened chart/project from a list
and then open it.
Then prompt according to this pseudocode:

```
if the user chooses a chart:
  if the chart is just the currently open chart:
    Do nothing.
  else if the chart was in a project:
    if its project is the same as the currently open project:
      The chart will be set as the active chart of the currently open project.
    else:
      prompt: Do you want to open the chart in its project?
      if yes:
        The project containing the chart will be opened, and its active chart will be set to the selected chart.
      else if a project is currently open:
        prompt: Do you want to add the chart to the current project?
        if yes:
          The chart (and its referenced music and image) will be added to the currently open project and will be opened in the project.
        else:
          The selected chart will be opened without opening a project.
      else:
        The selected chart will be opened without opening a project.
  else if a project is currently open:
    prompt: Do you want to add the chart to the current project?
    ...
    The selected chart will be opened without opening a project.
else if the selected project is the same as the currently open project:
  Do nothing.
else:
  The selected project will be opened.
```

After this, if the result is not doing nothing and there is currently unsaved changes,
prompt to save.

This operation is disabled in browser webpage.

#### Open auto-save...

Show a popup form to choose an auto-saved chart/project
and then open it.
If there is unsaved change, prompt to save.

#### Reload chart from disk

Open the chart from local filesystem again.
This option is grayed out if sviber is not in an NW.js app or the currently open chart does not have a known local file path.
If there is currently an open project, still work in the same project.
If there are unsaved changes, show a popup form saying that unsaved changes will be discarded.
...
The default value of the offset is the time of the first note (`tap`, `flick`, `hold`, `drag`).
The default value of the initial BPM is 60 divided by the time difference of
the first note after (exclusive) the first note and the first note.
...
The default charter is "RNOVA".
The difficulty-related fields use the same mechanics as normally created charts,
including the default values and the input listener to automatically change the difficulty color.

Every Lyrica channel (not including those ever used in the imported chart) corresponds to a sviber channel.
Set their names using their Lyrica channel numbers.
Import channel 100 of the Lyrica chart as if it is a bg note channel,
and import other disabled channels of the Lyrica chart as if they are normal channels,
but they are all imported as inactive channels.
...
If the saved chart is a new chart of the currently open sviber project,
also write to `project.sviber` accordingly.

#### Save as...

Save the chart to local filesystem after user selects a location in a file dialogue.
This operation does not concern with projects anyway.
Put a toast message after it completes.

#### Save project

Save the project.
Grayed out if there is not a currently open project.
If the project directory path is known, this operation is the same as [save](#save).
Otherwise, open a file dialogue for the user to select a directory to save to as the project folder,
in which case the directory does not have to be empty but must not have the `project.sviber` file.
Music files, image files, and all chart files will be saved to this directory if the same files are not already there.
Put a toast message after it completes.

#### Export level...

Generate a .ssc file and open a file dialogue to ask the user where to save it in the local filesystem.
The timestamps of all Zip entries should be normalized to zero for reproducibility.
If a project is open, the .ssc file contains all charts, all musics referenced by charts,
and all images referenced by charts;
otherwise, the .ssc file contains the current chart, the currently loaded music, and the currently loaded image.
The chart file in the .ssc file should not include sviber-specific information
(i.e., the top-level JSON object should not have the `sviber` field).
If there are text files in the project folder that would be shown as level readme texts in Sunniesnow
(e.g., `README.md`; see definition of `Sunniesnow.Utils.needsDisplayTextFile` in `../game-unstable/js/utils/Utils.js`),
also put them in the level file.
Put a toast message after it completes.
...
If a project is currently open, prompt to ask whether to add the chart to the project.
...
#### Render video..., Render cover...

Grayed out if not in NW.js app.
Show a popup form, confirming which
renders and saves the video or the cover image to local filesystem
using [sunniesnow-record](https://www.npmjs.com/package/sunniesnow-record).

When calling its API, generate the level file in the memory and provide it as a `Blob` object.
For rendering video, always use the currently loaded music as the selected music in the rendering
(so rendering video is grayed out if no music is loaded);
for rendering either, always use the currently loaded image as the background provided for sunniesnow-record,
and use `background: "none"` in the options if no image is loaded.
Also properly set `assetsDir` so that sunniesnow-record reuses the fonts bundled by sviber
...
Note that not every option accepted by sunniesnow-record should customizable.
...
- `width`, `height`: default 1920 and 1080.
- `fps`: default 60.
- `resultsDuration`: default 1.
- `waitForMusic`: default true.
...
- `coverThemeImageX`, `coverThemeImageY`, `coverThemeImageWidth`:
  ...
  The canvas has the same aspect ratio as the loaded image, and the loaded image is a controllable sprite.
  The user can move the sprite by dragging and zoom the sprite by scrolling.
...
and a text near it saying the current status of the rendering.
This popup form can only be confirmed after the rendering finishes.
When the rendering finishes, another button shows, clicking which opens the directory
containing the rendered file using the system file explorer.
...
Open a popup form for changing the filename of the chart in the project.
The input field is followed by uneditable ".json" text to indicate that the user must input the name without the filename extension,
and the filename extension would always be `.json` anyway.
The input is unacceptable if a file with the same name is already present in the project folder.
This action is immediately carried out in the local filesystem and cannot be undone.
This action is grayed out if a project is not currently open.

#### Edit chart properties...

Open a popup form for editing the same information asked when [creating a chart](#new)
for the currently open chart.
After the user confirms, if a project is currently open and the difficulty name is changed, open a new popup form
asking whether the user wants to rename the chart to `${difficultyName}.json`
(only if the new name does not cause a filename collision in the project folder).

#### Delete chart...

Open a popup form for user confirmation.
In the popup form, put a checkbox for whether to delete the chart file from the filesystem.
...
If the checkbox is checked, additionally delete the chart file from filesystem.
Open another chart in the project without asking the user which chart to open.

The change in the chart list and the active chart is immediately written to `project.sviber` in the local filesystem.

This item is grayed out if there is not a currently open project
or if this is the only chart in the project.

#### Close chart/project

Close the currently open chart/project.
...
Show a popup form for setting which [checks](#chart-checks) are enabled in this chart
and their parameters.
Put a checkbox for enabling and disabling each type of checks.
The input field for parameters are disabled for checks that are not enabled.
...
- Adjust offset (`adjust-offset.svg`)
- Automatic timing...
- (separator)
...
Enter offset adjustment mode.
In the offset adjustment mode, clicking and dragging on the waveform drags the beat lines by changing the offset.
Clicking and dragging on the waveform while holding <kbd>Ctrl</kbd> drags the beat line (including subdivision beat lines) closest to the mouse by changing the BPM of the last BPM change
before (exclusive) the beat line
(or the initial BPM if there is no BPM change before the closest beat line).
Clicking on the waveform still sets the current time.
Clicking (and possibly dragging) on the waveform while holding <kbd>Alt</kbd> is the original behavior
of setting the current time.
Notice that there should not be a smallest threshold distance
over which the mouse must move for the dragging action on the waveform
because adjusting offset sometimes needs very fine movements.
User interactions with other parts of the timeline are not affected.
Mouse interactions with the waveform with modification keys are not affected.

Using this action again or hitting <kbd>Esc</kbd> exits this mode.

#### Automatic timing...

This operation is grayed out if no music is loaded.

Use a beat tracking algorithm to find the offset, the initial BPM, and the BPM changes according to the music automatically.
Read Chapter 6 of <cite>Fundamentals of Music Processing</cite>
(I have given you the PDF version of the book)
carefully and implement the algorithms.
I expect you to need ndarrays for scientific computing,
in which case you should implement such data structures yourself instead of introducing dependencies.

Although the book mentions that the accuracy of the onset detection is on the order of 10 ms,
you should aim for accuracy on the level of 1 ms on some onsets because music for rhythm games
often has very clear onsets.
Obviously, though, it is impossible to have 1 ms accuracy for all onsets,
but it is usually possible to have 1 ms accuracy for the full beat sequence.

Show a popup form with the following input fields.
Because of the technical nature of the input fields, the texts in the tooltip bar for them
should be clear and detailed so that users without knowledge of music processing
(but with knowledge of mathematics and signal processing) can understand what each input field is.

First, use radio inputs for choosing an algorithm for onset detection
(i.e., how the novelty function is found).
Available options are energy (which is the default because it is suitable for music with regular percussions),
spectral, phase, and complex domain.

Then, use radio inputs for choosing an algorithm for tempo analysis
(i.e., how the tempogram is found).
Available options are short-time Fourier transform and short-time autocorrelation.

Then, use radio inputs for choosing an algorithm for beat tracking
(i.e., how the beat sequence is found).
Available options are predominant local pulse and dynamic programming.

The popup form should also have input fields for every parameter (e.g., various window widths, STFT hop sizes, logarithmic comporession factor $\gamma$,
BPM range $\Theta$, penalty weight $\lambda$, etc.),
except for those that are impractical to be fully customizable, such as the window functions,
for which you can just have radio inputs or `<select>` for choosing types of window functions.
Radio inputs are also suitable for some other parameters,
such as the estimated global BPM used for the dynamic programming algorithm,
for which radio inputs can be used to choose whether the user manually inputs the estimated global BPM
or the algorithm uses the BPM calculated from averaging over the tempogram.
The input fields for parameters should be folded in `<details>` as the user usually does not care about them
and should just use sensible default values.
The `<details>` element for parameters not applicable to the selected algorithm should be completely hidden.
The general structure is that
below each radio input for choosing an algorithm is a `<details>` element,
which is invisible when the radio input is not checked.

In the popup form, also notice the user that he will need to wait for a few seconds for the automatic timing algorithm to run.

After following the algorithms in <cite>Fundamentals of Music Processing</cite>,
you should get a beat sequence $\{b_l\}$ and the confidence level $p_l$ for each beat in the beat sequence.
For PLP, the confidence level is the peak height in the PLP function
(with properly normalized window function so that a peak close to 1 means good confidence);
for DP, the confidence level is the score that the beat contributes
(i.e., $p_l=\tilde p_l/\max\{\tilde p_l\}$, where
$\tilde p_l=|\Delta(b_l)+\frac\lambda2(P_{\hat\delta}(b_l-b_{l-1})+P_{\hat\delta}(b_{l+1}-b_l))|_{\ge0}$).
However, the timing information that the chart needs is the offset, the initial BPM, and the BPM changes
(do not worry about bar lines because the user is advised not to rely on the automatic timing
for the case where bar lines are necessary).
The naive approach is to set the offset to be the first beat in the beat sequence
and add a BPM change at every beat to make sure that the beat lines lie exactly at the beats in the beat sequence.
This approach is inappropriate:
the beat sequence must have small noises that make this approach result in a different BPM value for almost every beat
while the BPM changes only a few times throughout the music in most cases.
Denote the denoised beat sequence to be $\{\tilde b_l\}$, and it is the sequence that minimizes
$\frac12\sum_l|p_l(\tilde b_l-b_l)^2-\tilde q|_{\ge0}+\tilde\lambda\operatorname{TV}\{\tilde\delta_l\}$,
where TV denotes the total variation of a sequence, $\tilde\delta_l=\tilde b_l-\tilde b_{l-1}$, $\tilde\lambda$ is a customizable penalty weight,
and $\tilde q$ is a customizable tolerance for adjusting beats without penalty.
You may use a variant of the taut string algorithm to solve this optimization problem.
For a positive $\tilde q$, the solution may not be unique, in which case any one solution is okay.
The purpose of positive $\tilde q$ is to make sure $\{\tilde\delta_l\}$ has exact constant substrings (up to floating point errors),
which are segments of constant BPM.
If $\{\tilde\delta_l\}_{l\le l_1}$ and $\{\tilde\delta_l\}_{l>l_1}$ are constant
and $\tilde\delta_{l_1+1}-\tilde\delta_{l_1}\ne0$,
then it means that there is a BPM change exactly at $b_{l_1}$.
If $\{\tilde\delta_l\}_{l<l_1}$ and $\{\tilde\delta_l\}_{l>l_1}$ are constant
and $\operatorname{sgn}(\tilde\delta_{l_1}-\tilde\delta_{l_1-1})=\operatorname{sgn}(\tilde\delta_{l_1+1}-\tilde\delta_{l_1})\ne0$,
then it means that there is a BPM change between $b_{l_1-1}$ and $b_{l_1}$.

The implementation of the full algorithm should have high enough performance
so that a 10 minute music with 44.1 kHz sampling rate can be processed within a few seconds on a normal computer.
Still, to avoid freezing the webpage for too long,
run the algorithm in a worker thread.

After finishing the automatic timing, show a new popup form
showing new timing information (offset, initial BPM, and BPM changes).
There is a "Preview" button, and clicking it plays the music from the start
with metronome playing according to the new timing information.
The new timing information is only actually applied to the chart if the user confirms this popup form;
otherwise if the user cancels this popup form, the new timing information is discarded,
and the original timing information of the chart remains.

#### BPM change...

Show a popup form to enter information.
The only information to enter is the BPM number.
After the user confirms the popup form, a BPM change is created on the timeline at the current time.
If there is already a BPM change at the current time,
edit it instead.

Besides the confirmation button and the cancellation button,
there is a deletion button.
If the user clicks the deletion button, the BPM change at the current time is deleted.
...
- (separator)
- Activate (<kbd>A</kbd>)
- Deactivate (<kbd>Shift</kbd>+<kbd>A</kbd>)
- (separator)
- Fill curve with drag notes
- (separator)
- Bulk edit texts...
- Bulk edit texts by channel...
- Import lyrics/subtitle file...
...
key repetition from long pressing should be ignored)
...
Do not immediately play the SE for the newly created event.
If a `hold` event is created in this way,
also set its end time to the subdivision closest to the key up event
for the same key whose key down event triggered the creation of the `hold` event
(but if that subdivision is the same as the start time, use the next subdivision).
...
Lock or unlock all selected events.

Any event is unlocked by default.
If an event is locked, any operation that edits, transforms, or deletes selected events ignores it when it is selected.
Such operations behave as if locked events are not selected at all.
For example, [free transform](#free-transform) should be grayed out if all selected events are locked.
Locked events cannot be dragged in the timeline or the main editor field.
Locked events cannot be edited in the inspection panel.
The flick direction handles in the main editor field
and the duration tail handles in the timeline do not appear for locked events.

#### Activate, Deactivate

Activate or deactivate all selected events.

Any event is active by default.
If an event is inactive, it does not appear in the main editor field and the scroll view
and cannot be selected there.
Its SE does not play.
It cannot be connected by a tip point.
It still appears and can be selected in the timeline, but is transluscent.
...
Similar to [bulk edit texts by channel](#bulk-edit-texts-by-channel),
but it edits the texts of all selected textable events except `comment`,
and there is no `<select>` element in the popup form.
The sequence of edited events is sorted by time and then by channel.
Grayed out if no non-`comment` textable events are selected.
Texts of all selected textable events except `comment` are edited after this operation.
...
and selecting a different option in the dropdown selection does not discard the previously edited text:
the edits for different channels or the selection will be sumitted altogether when the user submits the form.
...
When the popup form opens and when the selected option in the `<select>` element changes to an option
that has not been edited since opening this popup form,
the cursor position is set to the text of the first textable event after (inclusive) the current time.
When the selected option in the `<select>` element changes to an option having been edited before
since opening this popup form,
the cursor position is set to where the user left it last time.

Texts of all textable evets except `comment` are edited after this operation.

#### Import lyrics/subtitle file...

Show a file dialogue to choose a lyrics/subtitle file to import.
Acceptable file formats are LRC, SRT, WebVTT, and ASS.
Then, show a popup form to enter an offset (default zero) and a quantization denominator.

For some of the formats (LRC with A2 extension, WebVTT with timestamp objects, and ASS with `\k` tags),
there can be timestamps marked within the line.
A line with such timestamps are imported as `tap` events in the current channel,
and a line without such timestamps are imported as a `bigText` event in the current channel.
For example, for the following WebVTT file:
...
`tap` events are created at respectively 12.340 s, 12.800 s, and 13.200 s
...
or $(-4+8j/(n-1))\cdot25$ if $n>9$, where $n$ is the total number of `tap` events in the line.
...
#### Move above within channel, Move below within channel

Change the order of events.
This is only meaningful if selected events are simultaneous with unselected events in the same channel.

For example, suppose that events A, B, C, and D are simultaneous events in the same channel,
and their stacking order as shown in the timeline from top to bottom is ABCD.
Then:

- Moving above within channel while selecting B, C, and D makes the new order BCDA.
- Moving above within channel while selecting B and D makes the new order BADC.
- Moving above within channel while selecting C and D makes the new order ACDB.
- Cannot move above within channel while selecting A.

This option is grayed out if no events are selected or some selected events cannot be moved above/below within the channel.
...
Delete the current channel along with all events (even locked ones) in this channel.
Change the current channel to the channel above it.
...
Typeset the rows and columns so that the rows and columns are well-aligned,
and do not have grid lines in the left column.
...
- Preset snappee...
- (separator)
- Activate snappee (<kbd>Ctrl</kbd>+<kbd>Shift</kbd>+<kbd>D</kbd>) (`../maker/svg/icons/activate-snappee.svg`)
- Deactivate snappee (<kbd>Ctrl</kbd>+<kbd>D</kbd>) (`../maker/svg/icons/deactivate-snappee.svg`)
- Deactivate all snappees (<kbd>Ctrl</kbd>+<kbd>Alt</kbd>+<kbd>D</kbd>)
- (separator)
- Attach (<kbd>S</kbd>) (`../maker/svg/icons/attach.svg`)
- Detach (<kbd>Shift</kbd>+<kbd>S</kbd>) (`../maker/svg/icons/detach.svg`)
- Attach to curve by order
- Attach to curve by time
- (separator)
- Copy snappee
- Paste snappee
...
When placing the first control point, a translucent control point handle appears under the mouse, possibly snapped to a snap point,
as the preview of its position.
Besides the preview of the resultant B&eacute;zier curve,
the control points are connected by a broken line.
...
When drawing a B&eacute;zier curve, the status panel shows
the coordinates of the next control point according to the current position of the mouse, considering snapping.
...
When placing the first two control points, a translucent control point handle appears under the mouse, possibly snapped to a snap point,
as the preview of its position.
...
When drawing a arc curve, the status panel shows the SVG path data of the curve.
It also shows the coordinates of the next control point according to the current position of the mouse, considering snapping.
...
When placing the first control point, a translucent control point handle appears under the mouse, possibly snapped to a snap point,
as the preview of its position.
When drawing the pen curve, besides the preview of the resultant curve,
the higher-order control points are connected to their respective first-order control points
(the control points that the pen curve is supposed to pass through) by line segments.
...
When drawing a pen curve, the status panel shows the SVG path data of the curve.
It also shows the coordinates of the next control point according to the current position of the mouse,
...
#### Activate snappee, Deactivate snappee

If there are events selected, activate or deactivate all snappees that at least one of the selected events are attached to;
otherwise, if there is a snapee selected, activate or deactivate it.
These items are grayed out if no events nor a snappee is selected.

#### Deactivate all snappees
...
Attach selected movable events to a curve. The curve is the curve selected by the user,
or if that does not exist, the only active curve.
If there is neither a curve selected nor only one active curve,
or if no movable events are selected, this item is grayed out.

The attachment is determined by the time ordering of selected movable events.
For simultaneous events, their order is determined by their would-be order in the exported chart file.
In other words, for simultaneous events, events in lower channels are ordered after events in upper channels;
for events stacked at the same time in the same channel,
events stacked at the bottom are ordered after events stacked at the top.
The earliest event is attached to snap point $i=0$, and the rest goes on to $i=1$, $i=2$, etc.
For example, suppose that the selected movable events are respectively at $t=1$, $t=1+1/2$, $t=3$, $t=3$, and $t=4$,
and then after this operation, they are attached to the snap points
$i=0$, $i=1$, $i=2$, $i=3$, and $i=4$, repectively.
...
The attachment is determined by the times of selected movable events.
Suppose that the time of the $k$th selected movable event is $t_k$ (in beats),
with the time of the earliest selected movable event being $t_0$.
Then, the snap point that the $k$th event will be attached to is $i=(t_k-t_0)/\Delta t$,
where $\Delta t=\operatorname{gcd}\left\{t_k-t_l\,\middle|\,k,l=0,1,\ldots\right\}$.
For example, suppose that the selected movable events are respectively at $t=1$, $t=1+1/2$, $t=3$, $t=3$, and $t=4$,
and then after this operation, they are attached to the snap points
$i=0$, $i=1$, $i=4$, $i=4$, and $i=6$, repectively.

#### Copy snappee

Copy the JSON data of the selected snappee to the clipboard.
Grayed out if no snappee is selected.

#### Paste snappee

Create a new snappee according to the JSON data in the clipboard.
...
- Flip horizontally with reattachment (<kbd>Ctrl</kbd>+<kbd>%</kbd>)
- Flip vertically (<kbd>"</kbd>) (`flip-vertically.svg`)
- Flip vertically with reattachment (<kbd>Ctrl</kbd>+<kbd>"</kbd>)
...
#### Flip horizontally, Flip horizontally with reattachment, Flip vertically, Flip vertically with reattachment
...
For the operations without reattachment, if some events are attached to snappees,
the whole snappees are also transformed (by editing their transformation matrix);
for the operations with reattachment, all attached events are detached before the transformation,
and after the transformation they are reattached to their respective original snappees
at new snap points (whichever are cloesest to them after the transformation).
Grayed out if no events are selected.
...
only handles for nondegenerate dimensions exists.
For example, if the selected events are two unattached notes with the same x coordinate, in which case the bounding box is just a vertical line
but not a 2D box,
then only the handles for scaling in the vertical direction and rotation are present.
If the bounding box is fully degenerate (e.g., if only one unattached movable event is selected),
...
The 6 input fields for the elements must be in a layout that make them form 3 columns in the conventional matrix form.
...
The 6 input fields must form 3 columns in the conventional matrix form.
...
Different from other input fields for rational numbers,
this popup form only have two input fields for the rational number,
the numerator and the denominator, where the numerator can be larger than the denominator.
...
Grayed out if there are no selected events.

#### Time translation...

Show a popup form that asks the user to enter a rational number.
Then, the times of all selected events are increased by this number of beats.
Grayed out if there are no selected events.
...
Show a popup form for the following input fields,
confirming which rounds the times and end times of all selected events
to the nearest subdivision.

- The subdivision denominator to quantize to.
  By default it is the current subdivision.
- What to do if there are two nearest subdivisions.
  Possible options are towards negative infinity, towards positive infinity,
  and to even subdivision.
...
- Set speed to 0.1 (<kbd>Ctrl</kbd>+<kbd>&grave;</kbd>) (`speed-0-1.svg`)
...
When playing, the sound effects of the notes and bg notes also play.
Check `../game-unstable/js/audio/se/SeWithMusic.js` to see how to make the sound effects perfectly in sync with the music.
Sunniesnow does not have SE for bg notes, but you should use the following synthetic sound for bg notes:
$$f(t)=\sin(2\pi\cdot1000t)\frac{\sin(2\pi\cdot100t)}{2\pi\cdot100t}\max(\operatorname{sgn}(0.02-t),0)$$

This menu item is still functional even if there is no music loaded.
In this case, only the note SE, the bg note SE, and the metronome (if enabled) play.
...
The music does not actually play, so there will only be note SE, bg note SE, and metronome (if enabled).
...
The playback rate cannot be lower than 0.1.

#### Set speed to 0.1, Set speed to 0.25, Set speed to 0.5, Set speed to 1

Directly set the playback rate.

There are keyboard shortcuts from <kbd>Ctrl</kbd>+<kbd>1</kbd> to <kbd>Ctrl</kbd>+<kbd>9</kbd>, for nine different playback rates,
but only some of them are available in the submenu so as not to clutter the submenu.
They are available in the [keyboard shortcuts](#keyboard-shortcuts) list, however.
...
- Macros... (`macros.svg`) (<kbd>Ctrl</kbd>+<kbd>Alt</kbd>+<kbd>M</kbd>)
- Run macro...

"Macros..." opens a real popup (for browser, use `window.open(url, target, 'popup')`) that contains the [macros interface](#macros-interface).

"Run macro..." opens a popup form that ask the user to choose a macro to run.
A radio input choses whether it is a global macro or a project macro.
The selection is remembered as the next default choice until sviber exits.
...
On MacBook, <kbd>Ctrl</kbd> is replaced by <kbd>Command</kbd>,
and <kbd>Alt</kbd> is replaced by <kbd>Option</kbd>.
This should be reflected in this list,
as well as the submenu items, the tooltips, and the help documentation.
...
- Background patterns: `bigText`, `grid`, `hexagon`, `checkerboard`, `diamondGrid`, `pentagon`, `turntable`, `hexagram`.
...
The tip point mode consists of the following parameters, which can be edited in the inspection panel:
...
When the user edits the tip point mode in the inspection panel while selecting multiple tip-pointable events,
the tip point modes for all of them are changed to be the same.
For example, if the user selects multiple `tap` events and set the tip point spawn type to "chain" in the inspection panel,
the tip point spawn types of all those selected `tap` events are changed into "chain".
...
(for this section, we only consider active tip-pointable events,
...
This section explains all types of checks,
...
This check checks that title, artist, and charter in the chart metadata are not empty.
Clicking the violation opens the popup form for [editing metadata](#edit-chart-properties).

### Irregular difficulty

This check checks the following:

- The difficulty name is one of "Easy", "Normal", "Hard", "Master", and "Special".
- The difficulty color is the correct corresponding one for the difficulty name
  (the automatically filled preset color when [creating a chart](#new-chart)).
- If the difficulty name is one of "Easy", "Normal", "Hard", and "Master",
  then the difficulty must be a valid positive integer;
  otherwise, the difficulty must be either a valid positive integer or a single character.
- If the difficulty name is an integer greater than 6, then the difficulty superscript
  must be either empty or "+"; otherwise, the difficulty superscript must be empty.

Clicking the violation opens the popup form for [editing metadata](#edit-chart-properties).

### Too many required fingers

This check checks that the chart is canonically playable with $n$ fingers,
where $n$ is a parameter of this check with a default value of 2.

This check assumes the canonical play style:
for every note (`tap`, `hold`, `flick`, `drag`),
a finger hits it right at the center right at the event time,
and the finger releases no earlier than the event end time,
and a finger can only move at a finite speed.

A `tap`, `hold`, or `flick` requires a finger down at the event time,
so $m$ simultaneous events of these three types requires $m$ fingers.

A `drag` does not require a finger down but a finger positioned at the event position
at the event time.
Therefore, if a `tap` and a `drag` are simultaneous,
they require 1 finger if they are at the same position,
but they require 2 fingers if they are at different positions.

A `hold` requires the exact finger that hit it to release no earlier than its end time,
so another finger is required to hit other non-`drag` notes during a `hold`.
For example, suppose that the chart consists of a `hold` from time 1 to time 3
and a `tap` at time 2, then this chart requires 2 fingers.
A chart consisting of a `hold` from time 1 to time 2
and a `tap` at time 2 also requires 2 fingers.
However, the finger that hits the `hold` can move freely during the `hold`,
so a chart consisting of a `hold` from time 1 to time 3
and a `drag` at time 2 only requires 1 finger.

When a violation is clicked in the checks panel,
select the event that requires an $(n+1)$th finger.
...
This check checks that all `hold` events have a duration at least $T$ seconds,
where $T$ is a parameter.
The default value of $T$ is 0.1.
There is a small tolerance for floating point errors.
When a violation is clicked,
select the too short event.

### Too short bg patterns

This check checks that all bg patterns (`bigText`, `grid`, `diamondGrid`, etc.)
have a duration of at least $T$ seconds, where $T$ is a parameter defaulting to 0.1.
There is a small tolerance for floating point errors.
When a violation is clicked,
select the too short event.

### Too short-lived tip points

This check checks that, for each tip point,
the time when it spawns and the time of the last event that it connects to
is at least $T$ seconds, where $T$ is a parmeter defaulting to 0.3.
There is a small tolerance for floating point errors.
When a violation is clicked,
select the first event that the tip point connects to.

### Sharp turns of tip points

This check checks that a tip point never makes a $\pi$ turn.
Because of floating point errors, a value very close to $\pi$ is also a violation.
When doing this check, watch out for the case where two consecutively connected events
are at the same time and the same position,
in which case they should be regarded as one turning point but not multiple turning points for the tip point.
When a violation is clicked,
the tip-pointable event at which the tip point makes a sharp turn is selected.

### Teleporting tip points

This check checks that, whenever two simultaneous events are connected by the same tip point,
they must be at the same position.
When a violation is clicked, the simultaneous events are selected.
...
This check checks that no event is earlier than the start of the music
or ends later than the end of the music.
When a violation is clicked, the violating event is selected.

### Drag screening

In Sunniesnow, one of the judgement settings is whether to enable Lyrica 5 mechanics.
With Lyrica 5 mechanics, an unjudged `drag` note can screen a touch down from hitting another note.
This check checks that, the drag screening does not make a note that is hittable without Lyrica 5 mechanics
become unhittable with Lyrica 5 mechanics.

A violation to this check is a `drag` event at time $t_0$ (seconds) that satisfies both the conditions:
(1) there is no other note (`tap`, `hold`, `drag`, `flick`)
that is both within spatial distance $L$ from the `drag` event
and is at a time (seconds) between (inclusive) $t_0-T$ and $t_0$;
(2) there is some other non-`drag` note (`tap`, `hold`, `flick`)
that is both within spatial distance $L$ from the `drag` event
and is at a time (seconds) between (inclusive) $t_0$ and $t_0+T$.
Here, the screening time $T$ and the screening distance $L$ are parameters of this check,
whose default values are respectively 0.4 and 40.

When a violation is clicked, the violating `drag` event is selected.

### Simultaneous overlapping notes

This check checks that no simultaneous notes except `drag` have the same position in space.
Becasue of floating point error, very close positions are also regarded as the same position.

There is a boolean parameter of check.
When it is true, only overlapping notes with truly invisible note bodies are regarded as violations.
This includes:

- Two simultaneous `hold` events with the same position.
- Two simultaneous `flick` events with the same position and the same
  (with small tolerance for floating point error) angle.
- Two simultaneous `tap` events with the same position while
  there is also another `tap` event that is simultaneous with them but does not necessarily have the same position.
- A `hold` event and a `tap` event that are simultaneous and have the same position,
  where the `tap` event is drawn behind the `hold` event.
...
### Conflicting bg patterns

For an event $E$, denote its time $t_0(E)$ and its end time $t_1(E)$.
If two background patterns $E_1$ and $E_2$ satisfy
$t_0(E_2)\le t_1(E_1)$ and $t_0(E_1)\le t_1(E_2)$, then they are conflicting background patterns.
This check checks that there are no conflicting background patterns.
When a violation is clicked, both background patterns are selected.

There is a boolean parameter for this check,
setting which to false makes the check less strict:
the condition of confliction becomes $t_0(E_2)<t_1(E_1)$ and $t_0(E_1)<t_1(E_2)$.
...
subtract the current audio context time
from the audio context time of the closest metronome beat to get a datum
...
The Monaco editor needs to be aware of the API and to support basic autocompletion.
...
- `::selected_events`: array of `Event` objects.
- `::offset`, `::offset=`.
...
- `events`: an array of objects containing information of events,
  including notes, `bgNote` events, background patterns, comments, and `group` events.
- `snappees`: an array of objects containing information of snappees.
- `clips`: an array of objects containing information of clips.
- `checks`: enabled checks and their parameters.
...
These fields should use absolute path if the chart is saved without a project,
but they should use relative path in the project folder
if the chart is in a project.
...
- `showHud`: boolean.
- `showBgEventsInMainField`: boolean.
- `showBgEventsInTimeline`: boolean.
- `mainFieldPanX`: number.
- `mainFieldPanY`: number.
- `mainFieldZoom`: number.
- `showChartBoundary`: boolean.
...
The `checks` field is an object containing the following fields.
Each of the following field is an object containing at least the field `enabled`,
and it possibly contain extra fields for parameters.
...
- `blockedTexts`.
- `conflictingBgPatterns`: extra field `strict`.
...
The difference between type `1` and type `2` tap is that
type `2` should be used if and only if there is another simultaneous tap event in the chart,
but some legitimate Lyrica charts do not follow this rule.
What you should do is to treat them as the same thing when importing a Lyrica chart
and to follow the rule that simultaneous taps should use type `2` when exporting a Lyrica chart.
...
The relation between a sviber chart and a sviber project is that,
a project contains one and more charts, and possibly a music and possibly an image.
Sviber can either be editing a chart or a project.

A sviber project lives in a local filesystem directory called the project folder.
For this reason, the browser webpage version of sviber does not support editing a project,
but only support editing a chart.

In the project directory, a file `project.sviber` is present.
It has the following fields:

- `charts`: array of charts; each chart has fields `file` and `id`.
- `activeChart`: the ID of the currently open chart in the project.
- `macros`: array of macros; each macro has fields `file` and `name`.

This file should not include information about musics and images
because different charts may use different musics and images.
...
Although there is the concept of a project,
Main editing functions are still focused on editing only one chart
(note that we are talking about the main editing functions here,
not including macros editing functions, which is a separate thing that is discussed in [macros interface](#macros-interface)).
Only the currently open chart can have unsaved changes, but there is no unsaved changes on the project level,
nor can charts other than the currently open chart have unsaved changes.
Therefore, if the user switches to a different chart in the currently open project
and chooses "no" when prompted to save unsaved changes,
the unsaved changes to the previously open chart are discarded.
Another consequence is that any modifications to the project but not to the chart
(what charts there are in the project, which chart is the active chart,
and what macros there are in the project)
are immediately written to the local filesystem.
...
Four languages need to be supported: zh-TW, zh-CN, en-US, and ja-JP.
Choose the default language according to `nagivator.language`, and use en-US as the fallback language.

The language names in the language settings should be written in their own languages
but not change with the interface language.
For example, zh-CN should be called "简体中文" whether the interface language is zh-CN or not.

Some special translations: artist is "曲师" in Chinese;
chart is "谱面" in Chinese;
charter is "谱师" in Chinese;
bg notes are "墨点" in Chinese;
...
Put these two tasks in two separate workflow files.

In the GitHub release assets, provide
...
in which case [rendering video](#render-video-render-cover) can only use FFmpeg from `PATH`.
This environment variable is useful for, e.g., building the Nix package,
for which the FFmpeg binary should be provided by using `--prefix PATH :` in `makeWrapper` arguments.

The NW.js app can be used to open files with `.sviber`, `.ssc`, `.json`, and `.txt` extensions.
The Linux distribution should include the MIME type definition XML file for `.sviber` and `.ssc`,
and the desktop entry file have relevant contents for opening these file types.
The macOS distribution should have `Info.plist` include the relevant contents for opening these file types.
The Windows distribution should have an Inno setup script for setting up the relevant registry fields.
...
Linting errors out if any of the following rules is broken:

- One JS file cannot exceed 1000 lines (not including comments).
- One JS function cannot exceed 100 lines (not including comments).
- One line cannot have more than one statement.
- One line cannot have more than 120 characters.
- There must be an empty line between two function definitions.
- `if` and `for` clauses must not omit curly braces.
- A ternary operator cannot span multiple lines.

When breaking up files and functions to comply with the rules,
the breaking must be conceptually non-trivial.
For example, simply separating a `app-2.js` out from `app.js` is not acceptable.

### Command line arguments

The NW.js app can also use the command line arguments provided by the user.
If a path is provided in argv, open or import a chart/level/project folder.
If the user uses sviber to open a file/directory while a sviber instance is already running, create a new instance.

The NW.js app can also be used as a purely CLI app for basic file manipulations.
Example usage:

- `sviber chart-or-project --export output.ssc` exports a Sunniesnow level based on the input path
  of a chart file or a project folder.
- `sviber chart-or-project --render output.mkv ...` renders video.
  The dots are additional parameters such as the nickname and the avatar.
- `sviber chart-or-project --render output.png ...` renders image.
  The dots are additional parameters such as the cover theme image area.
- `sviber chart --export output.json` exports a Sunniesnow chart based on the input path.
- `sviber chart --export output.txt` exports a Lyrica chart based on the input.
- `sviber json-or-txt-or-ssc --import output.json ...` imports a chart or level file and saves as sviber chart.
  The dots are parameters for importing such as the random seed.
  Devise proper CLI flags for the parameters.
- `sviber json-or-txt-or-ssc --import output-dir ...` imports a chart or level file and saves as sviber project.
  Devise proper CLI flags for the parameters.
