...
Do not put `.ico` format icon in the source repo.
...
Additionally, there is an additional segment of the thick white line emanating from the first event connected by the tip point,
...
- Lock visible range: when checked, the visible range can only be changed by interacting with the timeline scroll bar or the keyboard,
  ...
  or [seeking](#seek-forward-seek-backward). This is false by default.
...
1. Rectangular mesh from $(-100,50)$ to $(100,-50)$ with 16 by 8 tiles.
...
All these preset snappees except the first one are initially inactive.
...
- (separator)
- Flip horizontally (<kbd>%</kbd>)
- Flip vertically (<kbd>"</kbd>)
...
Each macro is simply a JavaScript script.
...
and project macros are stored as `.js` files in the project folder.
...
and whether it is a global macro or a project macro.
...
the currently open macro to a local file path as a JavaScript file.
...
the `console` global object is overridden to direct all outputs to the DOM console in the macros interface.
...
Use [Monaco](https://microsoft.github.io/monaco-editor/) to power a JavaScript editor.
...
You need to devise an API that JavaScript codes in macros can use to modify sviber's state.
It must be thorough enough to read and write every aspect of current chart data,
...
changing the current time by dragging on the waveform, etc.).
...
- Whenever a tag of the format `v*.*.*` is pushed, create a GitHub release,
  with redistributable NW.js apps for Windows, Linux, and macOS in the release assets.
