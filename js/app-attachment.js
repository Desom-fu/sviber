...
class AttachmentTrait {
	...
	#attachAtIndices(curve, ordered, indices, label) {
		const points = this.#curveSnapPoints(curve);
		if (!points.length) {
			return false;
		}
	...
	attachSelectedToCurveByOrder() {
		...
		return this.#attachAtIndices(curve, ordered, indices, i18n.t("command.snappee.attachCurveOrder"));
	}
	...
	attachSelectedToCurveByTime() {
		...
		return this.#attachAtIndices(curve, ordered, indices, i18n.t("command.snappee.attachCurveTime"));
	}
