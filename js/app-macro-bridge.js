...
export async function listRunnableMacros(app) {
	...
	if (globalThis.nw && app.files?.projectPath) {
		...
		} catch { /* Browser and missing project folders yield no project macros. */ }
	}
...
async function loadRubyResources() {
	...
	if (!rubyResponse.ok) throw new Error(`Ruby API HTTP ${rubyResponse.status}`);
...
export async function runChosenMacro(app, macroId) {
	...
	if (!chosen) throw new Error("The selected macro is unavailable.");
	let content = chosen.content;
	if (chosen.scope === "project") {
		content = await app.files.readProjectText(chosen.filename);
		if (content == null) throw new Error("The project macro could not be read.");
	}
	let rubyResources = {};
	if (chosen.language === "ruby") rubyResources = await loadRubyResources();
	...
	const result = await new Promise(resolve => {
		...
		const listener = event => {
			...
			if (event.data?.type === "result" || event.data?.type === "error") finish(event.data);
		};
		window.addEventListener("message", listener);
		frame.addEventListener("load", () => frame.contentWindow.postMessage({
			type: "run", code: content, state: deepClone(app.model.snapshot()),
			language: chosen.language, ...rubyResources,
		}, "*"), { once: true });
	...
	frame.remove();
	if (result.type === "error" || !result.state) throw new Error(result.message || "The macro failed.");
