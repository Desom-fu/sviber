...
export function sunniesnowHitSample(type, time) {
	...
	if (type === "bgNote") {
		if (value >= BG_NOTE_SE_DURATION) {
			return 0;
		}
		const argument = 2 * Math.PI * 100 * value;
		const envelope = argument === 0 ? 1 : Math.sin(argument) / argument;
		return Math.sin(2 * Math.PI * 1000 * value) * envelope;
	}
	if (type === "drag") {
...
export function createSunniesnowHitSamples(type, sampleRate, duration = null) {
	const span = Number.isFinite(duration) && duration > 0 ? duration : type === "bgNote" ? BG_NOTE_SE_DURATION : 0.3;
	const result = new Float32Array(Math.max(1, Math.floor(span * sampleRate)));
...
export class AudioPlayer extends EventTarget {
	...
	// Chromium freezes AudioContext.currentTime for a while after several minutes of
	// pure silence. Feeding a constant source into a media stream destination keeps
	// the rendering thread hot so that the clock never stalls.
	#startKeepAlive() {
		const context = this.context;
		if (!context || this.keepAlive) {
			return;
		}
		...
		try {
			...
			keepAlive.connect(context.createMediaStreamDestination());
			keepAlive.start();
			this.keepAlive = keepAlive;
		} catch {
			/* Keeping the clock hot is best effort only. */
		}
	}
	...
	setSeVolume(volume) {
		...
		this.seVolume = Math.max(0, Math.min(2, parsed));
		if (this.seGain) {
			this.seGain.gain.value = this.seVolume;
		}
	}
	...
	setLoopRange(range) {
		...
		const next =
			values.length === 2 && values.every(Number.isFinite) && values[1] > values[0]? [values[0], values[1]]: null;
		if (JSON.stringify(next) === JSON.stringify(this.loopRange)) {
			return;
		}
		const wasPlaying = this.playing;
		const time = this.currentTime;
	...
	async #playDirection(direction) {
		...
		if (this.playing && this.direction === nextDirection) {
			return;
		}
		const generation = ++this.playbackGeneration;
		const context = await this.ensureContext();
		if (!context || generation !== this.playbackGeneration) {
			return;
		}
		const wasPlaying = this.playing;
		if (wasPlaying) {
		...
		this.playing = true;
		if (this.direction > 0 && this.position >= this.duration) {
			this.position = 0;
		}
		...
		if (wasPlaying) {
			...
			if (!this.source) {
				this.#startSource({ preserveClock: true });
			}
			this.#tick();
		}
	}
	...
	#startSource({ preserveClock = false } = {}) {
		...
		this.lastLoopCycle = 0;
		if (!this.buffer || this.direction < 0) {
			return;
		}
	...
	#stopHitSources(futureOnly = false) {
		...
		for (const record of this.hitSources) {
			if (futureOnly && record.startTime <= currentTime) {
				continue;
			}
	...
	#tick = () => {
		...
		const endedForward =
			!this.loopRange && this.direction > 0 && !this.buffer && playback.time >= this.syntheticEnd;
	...
	#startHit(type, delay, scheduledAt) {
		...
		if (!this.hitBuffers.has(sampleType)) {
			const samples = createSunniesnowHitSamples(sampleType, context.sampleRate);
