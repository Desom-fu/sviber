...
function collectIndexedSchedule(
	records,
	field,
	outputField,
	currentTime,
	...
	for (let index = lowerBound(records, minimum, field); index < records.length; index += 1) {
		...
		if (scheduledTime > horizon + 1e-8) {
			break;
		}
		if (scheduledTime >= maximumTime - 1e-8) {
			break;
		}
		if (scheduledTime < minimum - 1e-8) {
			continue;
		}
		if (scheduledIds.has(record.event.id)) {
			continue;
		}
		result.push({
			event: record.event,
...
export function collectHitSchedule(
	events,
	timing,
	currentTime,
	playbackRate,
	scheduledIds,
	lookAhead = HIT_LOOKAHEAD_SECONDS,
	lateTolerance = 0.02,
	maximumTime = Infinity,
	minimumTime = -Infinity,
) {
	...
	for (const event of events) {
		if (!HIT_SOUND_TYPES.has(event.type) || scheduledIds.has(event.id)) {
			continue;
		}
		const hitTime = timing.beatToSeconds(event.time);
		if (hitTime < minimum - 1e-8 || hitTime > horizon + 1e-8 || hitTime >= maximumTime - 1e-8) {
			continue;
		}
...
export function collectHoldReleaseSchedule(
	events,
	timing,
	currentTime,
	playbackRate,
	...
	const result = [];
	for (const event of events) {
		if (event.type !== "hold" || scheduledIds.has(event.id)) {
			continue;
		}
		const releaseTime = timing.beatToSeconds(event.time) + timing.durationToSeconds(event.time, event.duration);
		if (releaseTime < minimum - 1e-8 || releaseTime > horizon + 1e-8 || releaseTime >= maximumTime - 1e-8) {
			continue;
		}
...
export function collectIndexedHitSchedule(
	records,
	currentTime,
	playbackRate,
	scheduledIds,
...
export function collectIndexedHoldReleaseSchedule(
	records,
	currentTime,
	playbackRate,
	scheduledIds,
...
function collectReverseRecords(
	records,
	field,
	currentTime,
	playbackRate,
	...
	for (let index = lowerBound(records, horizon, field); index < records.length; index += 1) {
		...
		if (hitTime > maximum + 1e-8) {
			break;
		}
		if (hitTime <= minimumTime + 1e-8) {
			continue;
		}
		if (scheduledIds.has(record.event.id)) {
			continue;
...
export function collectReverseHitSchedule(
	events,
	timing,
	currentTime,
	playbackRate,
...
export function collectIndexedReverseHitSchedule(
	records,
	currentTime,
	playbackRate,
	scheduledIds,
...
export function collectMetronomeSchedule(
	timing,
	currentTime,
	playbackRate,
	direction,
	...
	let lines = integerBeatLines(currentBeat, endingBeat);
	if (typeof timing.beatLinesBetween === "function") {
		lines = timing.beatLinesBetween(currentBeat, endingBeat, 1);
	}
	if (!forward) {
		lines.reverse();
	}
	...
	for (const line of lines) {
		...
		if (scheduledBeats.has(beat)) {
			continue;
		}
		const time = timing.beatToSeconds(line.beat?.toJSON?.() ?? beat);
		if (
			Array.isArray(loopRange) &&
			loopRange.length === 2 &&
			(forward ? time >= loopRange[1] - 1e-8 : time <= loopRange[0] + 1e-8)
		) {
			continue;
		}
		const distance = forward ? time - currentTime : currentTime - time;
		if (distance < -0.02 || distance > Math.max(0, lookAhead) * rate + 1e-8) {
			continue;
		}
...
function integerBeatLines(beginning, end) {
	const start = Math.ceil(
		Math.min(Number(beginning?.toNumber?.() ?? beginning), Number(end?.toNumber?.() ?? end)) - 1e-9,
	);
	const finish = Math.floor(
		Math.max(Number(beginning?.toNumber?.() ?? beginning), Number(end?.toNumber?.() ?? end)) + 1e-9,
	);
