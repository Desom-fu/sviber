...
export function invertTransform(transform) {
	...
	if (Math.abs(determinant) <= EPSILON) {
		throw new RangeError("transformation is singular");
	}
	return [
		d / determinant,
		-b / determinant,
...
export function clampAffineToChartBounds(points, requested, previous = IDENTITY_TRANSFORM) {
	...
	if (!points.length || points.every(point => isPointWithinChartBounds(applyTransform(point, next)))) {
		return [...next];
	}
	let delta;
	try {
		delta = multiplyTransforms(next, invertTransform(from));
	} catch {
		return [...from];
	}
	const [a, b, c, d] = delta;
	...
	if (!points.every(point => isPointWithinChartBounds(applyTransform(point, from)))) {
		return [...from];
	}
	const rotation = rotationCenter(delta);
	const sample = t => {
		if (rotation && Math.abs(a - d) <= 1e-6 && Math.abs(b + c) <= 1e-6 && Math.abs(a * a + b * b - 1) <= 1e-3) {
	...
	if (points.every(point => isPointWithinChartBounds(applyTransform(point, matrix)))) {
		return matrix;
	}
	let lo = 0,
		hi = tMax,
...
export function transformAngle(angle, transform) {
	...
	if (Math.hypot(transformedX, transformedY) <= EPSILON) {
		return radians;
	}
...
// Calls a user-supplied expression function with the argument shape it declares: two-dimensional
// scopes (those carrying a `j` index) may take either `(i, j, scope)` or a single `scope`, while
// one-dimensional scopes always receive `(i, scope)`.
function callExpressionFunction(expression, scope) {
	if (!Object.hasOwn(scope, "j")) {
		return expression(scope.i, scope);
	}
	if (expression.length >= 2) {
		return expression(scope.i, scope.j, scope);
	}
...
function evaluateExpression(expression, scope, evaluator) {
	...
	if (typeof expression === "function") {
		return finiteNumber(callExpressionFunction(expression, scope), "expression result");
	}
	const text = String(expression ?? "").trim();
	if (Object.hasOwn(scope, text)) {
		return finiteNumber(scope[text], "expression result");
	}
	...
	if (typeof engine === "function") {
		return finiteNumber(engine(text, scope), "expression result");
	}
...
function sampleRectangularMesh(snappee) {
	...
	for (let i = 0; i <= horizontalTiles; i += 1) {
		for (let j = 0; j <= verticalTiles; j += 1) {
			result.push(
				point(left + ((right - left) * i) / horizontalTiles, top + ((bottom - top) * j) / verticalTiles, [
					i,
					j,
				]),
			);
...
function sampleRadialMesh(snappee) {
	...
	for (let i = 0; i < azimuthalTiles; i += 1) {
		...
		for (let j = 0; j <= radialTiles; j += 1) {
			...
			result.push(
				point(centerX + currentRadius * Math.cos(angle), centerY + currentRadius * Math.sin(angle), [i, j]),
			);
		}
	}
	return result;
...
function resamplePolyline(polyline, outputCount, closed = false) {
	if (polyline.length === 0 || outputCount <= 0) {
		return [];
	}
	if (polyline.length === 1) {
		return Array.from({ length: outputCount }, () => ({ ...polyline[0] }));
	}
	const source = polyline.map(({ x, y }) => ({ x, y }));
	if (closed && distance(source[0], source.at(-1)) > EPSILON) {
	...
	if (total <= EPSILON) {
		return Array.from({ length: outputCount }, () => ({ ...source[0] }));
	}
...
export function sampleSnappeePath(snappee, options = {}) {
	...
	if (snappee.type === "bezierCurve") {
		...
		const resolution = Math.max(
			256,
			positiveInteger(snappee.segments, "segments", 1) * 16,
	...
	if (snappee.closed && raw.length > 1 && distance(raw[0], raw.at(-1)) > EPSILON) {
		raw.push({ ...raw[0] });
	}
...
export function penCommandsFromNodes(nodes = [], closed = false) {
	...
	if (closed && normalized.length > 1) {
		appendSegment(normalized.at(-1), normalized[0]);
	}
	return commands;
}
...
// SVG path data for pen and circular arc curves, used by the status panel and by the
// copy button of the pen curve popup form (v17).
export function penCommandsToSvgPath(commands = [], closed = false) {
	const parts = [];
	for (const command of commands) {
		if (command?.type === "C") {
			parts.push(
				`C ${formatPathNumber(command.x1)} ${formatPathNumber(command.y1)}` +
					` ${formatPathNumber(command.x2)} ${formatPathNumber(command.y2)}` +
					` ${formatPathNumber(command.x)} ${formatPathNumber(command.y)}`,
			);
			continue;
		}
		if (command?.type === "M" || command?.type === "L") {
			parts.push(`${command.type} ${formatPathNumber(command.x)} ${formatPathNumber(command.y)}`);
		}
	}
...
export function svgPathToPenCommands(pathData) {
	const tokens = String(pathData || "").match(/[MmLlCcZz]|[-+]?(?:\d*\.\d+|\d+)(?:[eE][-+]?\d+)?/g) || [];
	const commands = [];
	let closed = false;
	let cursor = { x: 0, y: 0 };
	let index = 0;
	...
	while (index < tokens.length) {
		...
		if (type === "C") {
			const values = [nextNumber(), nextNumber(), nextNumber(), nextNumber(), nextNumber(), nextNumber()];
		...
		const base = relative ? cursor : { x: 0, y: 0 };
		commands.push({ type, x: base.x + x, y: base.y + y });
		cursor = { x: base.x + x, y: base.y + y };
	}
	return { commands, closed };
}

// A closed circular arc always spans a full turn; an open one stops at whichever end angle the
// snappee declares (`endAngle` is the current name, `endingAngle` the legacy one).
function circularArcEndingAngle(snappee, beginning, clockwise, closed) {
	if (closed) {
		return beginning + (clockwise ? -TAU : TAU);
	}
	return Number(snappee?.endAngle ?? snappee?.endingAngle ?? beginning);
}

function circularArcAngularSpan(beginning, ending, clockwise, closed) {
	if (closed) {
		return clockwise ? -TAU : TAU;
	}
	if (clockwise) {
		return -normalizedPositiveAngle(beginning - ending);
	}
	return normalizedPositiveAngle(ending - beginning);
}

export function circularArcSvgPath(snappee) {
	...
	const beginning = Number(snappee?.beginningAngle ?? 0);
	const clockwise = Boolean(snappee?.clockwise);
	const closed = Boolean(snappee?.closed);
	const ending = circularArcEndingAngle(snappee, beginning, clockwise, closed);
	const span = circularArcAngularSpan(beginning, ending, clockwise, closed);
	const from = { x: centerX + radius * Math.cos(beginning), y: centerY + radius * Math.sin(beginning) };
	if (closed) {
		const opposite = { x: centerX - radius * Math.cos(beginning), y: centerY - radius * Math.sin(beginning) };
		const sweep = clockwise ? 0 : 1;
		return (
			`M ${formatPathNumber(from.x)} ${formatPathNumber(from.y)}` +
			` A ${formatPathNumber(radius)} ${formatPathNumber(radius)} 0 0 ${sweep}` +
			` ${formatPathNumber(opposite.x)} ${formatPathNumber(opposite.y)}` +
			` A ${formatPathNumber(radius)} ${formatPathNumber(radius)} 0 0 ${sweep}` +
			` ${formatPathNumber(from.x)} ${formatPathNumber(from.y)} Z`
		);
	}
	const to = { x: centerX + radius * Math.cos(beginning + span), y: centerY + radius * Math.sin(beginning + span) };
	const largeArc = Math.abs(span) > Math.PI ? 1 : 0;
	const sweep = span >= 0 ? 1 : 0;
	return (
		`M ${formatPathNumber(from.x)} ${formatPathNumber(from.y)}` +
		` A ${formatPathNumber(radius)} ${formatPathNumber(radius)} 0 ${largeArc} ${sweep}` +
		` ${formatPathNumber(to.x)} ${formatPathNumber(to.y)}`
	);
}
...
export function sampleSnappee(snappee, options = {}) {
	...
	if (!sampler) {
		throw new TypeError(`Unsupported snappee type: ${snappee.type}`);
	}
	const transform = normalizeTransform(snappee.transformation ?? IDENTITY_TRANSFORM);
...
function snappeeCollection(snappees) {
	if (snappees instanceof Map) {
		return [...snappees.values()];
	}
	if (Array.isArray(snappees)) {
		return snappees;
	}
	if (snappees && typeof snappees === "object") {
		return Object.values(snappees);
	}
	return [];
}
...
function insideBounds(candidate, bounds) {
	...
	return (
		candidate.x >= (bounds.minX ?? -Infinity) - SNAP_BOUNDARY_EPSILON &&
...
export function findNearestSnapPoint(input, snappees, options = {}) {
	...
	for (const snappee of snappeeCollection(snappees)) {
		if (activeOnly && snappee.active === false) {
			continue;
		}
		for (const candidate of sampleSnappee(snappee, options)) {
			if (!insideBounds(candidate, options.bounds)) {
				continue;
			}
			const squared = (candidate.x - target.x) ** 2 + (candidate.y - target.y) ** 2;
			if (squared > nearestSquared || (nearest && squared === nearestSquared)) {
				continue;
			}
...
export function snapSnappeeTranslation(snappee, delta, snappees, options = {}) {
	...
	for (const candidate of snappeeCollection(snappees)) {
		if (candidate === snappee || (snappee?.id != null && candidate?.id === snappee.id)) {
			continue;
		}
		if (activeOnly && candidate.active === false) {
			continue;
		}
		for (const point of sampleSnappee(candidate, options)) {
			if (insideBounds(point, options.bounds)) {
				targets.push(point);
			}
		}
	}
	...
	for (const source of sampleSnappee(snappee, options)) {
		...
		for (const target of targets) {
			...
			if (squared > bestSquared || (correction && squared === bestSquared)) {
				continue;
			}
			bestSquared = squared;
