...
export function walkEvents(events, visitor, ancestors = []) {
	for (const event of Array.isArray(events) ? events : []) {
		...
		if (event.type === "group") {
			walkEvents(event.events, visitor, [...ancestors, event]);
		}
	}
}
...
export function flattenEvents(events, includeGroups = true) {
	...
	walkEvents(events, event => {
		if (includeGroups || event.type !== "group") {
			result.push(event);
		}
	});
	return result;
...
export function findEventContainer(events, id) {
	...
	const visit = (items, ancestors) => {
		for (const event of items || []) {
			if (event.id === id) {
				container = { items, ancestors };
				return true;
			}
			if (event.type === "group" && visit(event.events, [...ancestors, event])) {
				return true;
			}
		}
		return false;
...
export function groupSiblingEvents(events, ids, factory) {
	...
	if (ids.some(id => findEventContainer(events, id)?.items !== container.items)) {
		return null;
	}
	...
	for (let index = container.items.length - 1; index >= 0; index -= 1) {
		if (index !== first && selected.has(container.items[index].id)) {
...
export function eventChannels(event) {
	if (event?.type !== "group") {
		return event?.channel == null ? [] : [event.channel];
	}
	return [
		...new Set(
			descendants(event)
