...
export function normalizeBarLines(lines = []) {
	...
	for (const line of normalized) {
		if (deduplicated.at(-1)?.time.equals(line.time)) {
			deduplicated[deduplicated.length - 1] = line;
		} else {
...
function safeInteger(value, label) {
	...
	if (!Number.isSafeInteger(number)) {
		throw new RangeError(`${label} is outside the supported beat range`);
	}
	return number;
}
...
export class TimingMap {
	...
	addBarLine(time) {
		...
		if (this.barLines.some(line => line.time.equals(target))) {
			return false;
		}
	...
	latestBarLineAt(time) {
		...
		while (low < high) {
			...
			if (this.barLines[middle].time.compare(target) <= 0) {
				low = middle + 1;
			} else {
				high = middle;
			}
	...
	beatLinesBetween(beginning, end, subdivision = 1) {
		...
		for (let index = 0; index < actual.length; index += 1) {
			segments.push({
				base: actual[index],
				beginning: actual[index],
				end: actual[index + 1] ?? null,
				barLine: true,
			});
		}
		const result = [];
		...
		for (const segment of segments) {
			const rangeBeginning =
				segment.beginning && segment.beginning.compare(first) > 0 ? segment.beginning : first;
			const rangeEnd = segment.end && segment.end.compare(last) < 0 ? segment.end : last;
			if (rangeBeginning.compare(rangeEnd) > 0) {
				continue;
			}
			...
			for (let step = firstStep; step <= lastStep; step += 1) {
				...
				if (segment.beginning && beat.compare(segment.beginning) < 0) {
					continue;
				}
				if (segment.end && beat.compare(segment.end) >= 0) {
					continue;
				}
	...
	bpmAtBeat(beat) {
		...
		while (low < high) {
			const middle = (low + high) >> 1;
			if (this.bpmChanges[middle].time.compare(target) <= 0) {
				low = middle + 1;
			} else {
				high = middle;
			}
	...
	_integralBetween(beginning, end) {
		...
		for (const change of this.bpmChanges) {
			if (change.time.compare(cursor) <= 0) {
				continue;
			}
			if (change.time.compare(finish) >= 0) {
				break;
			}
	...
	secondsToBeatNumber(seconds) {
		const target = assertFinite(seconds, "seconds");
		if (this._changeTimes.length === 0) {
			return ((target - this.offset) * this.initialBpm) / 60;
		}
		...
		while (low < high) {
			const middle = (low + high) >> 1;
			if (this._changeTimes[middle].seconds <= target) {
				low = middle + 1;
			} else {
				high = middle;
			}
	...
	snapBeat(value, subdivision) {
		...
		if (nextBar && nextBar.sub(target).abs().compare(closest.sub(target).abs()) < 0) {
			closest = nextBar;
		}
		return closest;
	}
	...
	_rebuildChangeTimes() {
		...
		let split = 0;
		while (split < this.bpmChanges.length && this.bpmChanges[split].time.compare(0) <= 0) {
			split += 1;
		}
...
export function secondsToBeat(timing, seconds, maxDenominator) {
	return (timing instanceof TimingMap ? timing : new TimingMap(timing)).secondsToBeat(seconds, maxDenominator);
}
