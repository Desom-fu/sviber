...
function readGlobal() {
	try {
		...
		/* A damaged macro store is treated as empty. */
	}
}

function writeGlobal() {
	const value = Object.fromEntries(
		[...macros.global].map(([name, macro]) => [
			name,
			{
	...
	try {
		...
		appendConsole(t("error.globalStore"), "error");
	}
}
...
function uniqueName(name, type) {
	...
	if (!hasMacroName(macros[type], base)) {
		return base;
	}
	let index = 2;
	while (hasMacroName(macros[type], `${base} ${index}`)) {
		index += 1;
	}
	return `${base} ${index}`;
}
...
function macroKey(macro) {
	return `${macro.type}:${macro.name}`;
}

function validateMacroName(name, collection, language = "javascript") {
	const clean = macroName(name);
	if (!clean) {
		return t("error.emptyName");
	}
	if (!macroFilename(clean, language)) {
		return t("error.reservedName");
	}
	if (collection && hasMacroName(collection, clean)) {
		return t("error.duplicateName");
	}
	return "";
}
...
function showMacroForm({
	...
	return new Promise(resolve => {
		...
		const submit = event => {
			...
			const selectedScope =
				includeScope && projectAvailable && !readOnly && projectRadio?.checked ? "project" : "global";
		...
		if (projectRadio) {
			projectRadio.disabled = !projectWritable || !includeScope;
		}
		if (globalRadio) {
			globalRadio.checked = !includeScope || scope !== "project" || !projectWritable;
		}
		if (projectRadio) {
			projectRadio.checked = includeScope && scope === "project" && projectWritable;
		}
		if (rubyRadio) {
			rubyRadio.checked = includeLanguage && language === "ruby";
		}
		if (javascriptRadio) {
			javascriptRadio.checked = !includeLanguage || language !== "ruby";
		}
...
function currentTab() {
	return activeKey ? openTabs.get(activeKey) : null;
}
...
function refreshReadOnlyState() {
	...
	const projectRadio = elements.form?.querySelector('input[name="macro-scope"][value="project"]');
	if (projectRadio) {
		projectRadio.disabled = readOnly || !projectAvailable;
	}
...
function setEditorValue(value) {
	if (editorReady) {
		...
		elements.fallback.value = String(value);
	}
}

function markDirty() {
	const tab = currentTab();
	if (!tab || tab.loading || !tabIsEditable(tab)) {
		return;
	}
...
function activateTab(key) {
	...
	if (activeKey && openTabs.has(activeKey)) {
		openTabs.get(activeKey).macro.content = getEditorValue();
	}
	activeKey = key;
	tab.loading = true;
	...
	if (editorReady) {
		globalThis.monaco?.editor.setModelLanguage(
			editor.getModel(),
			tab.macro.language === "ruby" ? "ruby" : "javascript",
		);
		editor.layout();
	}
	queueMicrotask(() => {
...
function openMacro(macro) {
	...
	if (!openTabs.has(key)) {
		openTabs.set(key, { macro, dirty: false, loading: false });
	}
	activateTab(key);
}
...
function closeTab(key, force = false) {
	...
	if (!force && tab.dirty && !window.confirm(interpolate("message.discard", { name: tab.macro.name }))) {
		return;
	}
	openTabs.delete(key);
	if (activeKey === key) {
...
async function loadProjectMacros() {
	if (!projectAvailable || projectMacrosLoaded) {
		return;
	}
...
async function saveTab(tab) {
	...
	if (!tabIsEditable(tab)) {
		appendConsole(t("error.readOnly"), "error");
		return false;
	}
	tab.macro.content = getEditorValue();
	if (tab.macro.type === "global") {
		writeGlobal();
	} else {
		if (!projectAvailable) {
			appendConsole(t("error.projectUnavailable"), "error");
			return false;
		}
		...
		if (!result.ok) {
			appendConsole(t("error.saveProject"), "error");
			return false;
		}
	}
	tab.dirty = false;
...
function lastMacroLanguage() {
	try {
		...
		if (stored === "javascript" || stored === "ruby") {
			return stored;
		}
	} catch {
...
async function newMacro() {
	const values = await showMacroForm({
		titleKey: "form.newTitle",
		initialName: uniqueName("macro", activeList),
		includeScope: true,
		scope: activeList,
		includeLanguage: true,
		language: lastMacroLanguage(),
	...
	const content = language === "ruby" ? '# sviber macro\nputs "hello"\n' : '// sviber macro\nconsole.log("hello");\n';
	...
	if (type === "project") {
		...
		if (!result.ok) {
			appendConsole(t("error.createProject"), "error");
			return;
		}
	}
...
async function renameMacro() {
	...
	const values = await showMacroForm({
		titleKey: "form.renameTitle",
		initialName: tab.macro.name,
		includeScope: false,
		language: tab.macro.language,
		validate: name =>
			name === tab.macro.name ? "" : validateMacroName(name, macros[tab.macro.type], tab.macro.language),
	});
	if (!values || values.name === tab.macro.name) {
		return;
	}
	...
	if (tab.macro.type === "project") {
		...
		if (!result.ok) {
			appendConsole(result.error || t("error.renameProject"), "error");
			return;
		}
	}
...
async function handleImport() {
	...
	const values = await showMacroForm({
		titleKey: "form.importTitle",
		initialName: uniqueName(macroName(file.name), activeList),
		includeScope: true,
		includeLanguage: true,
	...
	if (type === "project") {
		const result = await request("sviber-macro-project-write", { filename, text: macro.content });
		if (!result.ok) {
			appendConsole(t("error.importProject"), "error");
			return;
		}
	}
...
async function exportMacro() {
	...
	const ruby = tab.macro.language === "ruby";
	const extension = ruby ? ".rb" : ".js";
	const result = await saveMacroFile(getEditorValue(), {
		filename: `${tab.macro.name}${extension}`,
		extension,
		mime: ruby ? "text/x-ruby" : "text/javascript",
		description: ruby ? "Ruby macro" : "JavaScript macro",
	});
	if (result.error) {
		appendConsole(`${t("error.export")} ${result.error?.message || result.error}`, "error");
		return;
	}
	if (result.saved) {
...
async function deleteMacro() {
	...
	if (!tab || !window.confirm(interpolate("message.delete", { name: tab.macro.name }))) {
		return;
	}
	if (tab.macro.type === "project") {
...
function loadRubyResources() {
	if (!rubyResourcesPromise) {
		...
		rubyResourcesPromise = Promise.all([
			fetch(new URL("js/macro/macro-api.rb", location.href)).then(response => {
				if (!response.ok) {
					throw new Error(`Ruby API HTTP ${response.status}`);
				}
...
async function runMacro() {
	...
	const tab = currentTab();
	if (!tab) {
		appendConsole(t("error.noMacro"), "error");
		return;
	}
	tab.macro.content = getEditorValue();
	...
	const result = await new Promise(resolve => {
		...
		const listener = event => {
			...
			if (event.data?.type === "log") {
				appendConsole(event.data.values.join(" "), event.data.kind);
			}
	...
	if (result.type === "error") {
		appendConsole(result.message, "error");
		return;
	}
	const applied = await request("sviber-macro-apply", { state: result.state });
	if (applied.ok) {
		appendConsole(t("message.applied"));
	} else {
		appendConsole(applied.error || t("error.noState"), "error");
	}
}
...
function installMenus() {
	...
	document.querySelectorAll("[data-action]").forEach(button =>
		button.addEventListener("click", () => {
			...
			const editAction = action => {
				...
				if (editorReady) {
					editor.trigger("menu", commands[action] || action);
				} else {
	...
	for (const button of document.querySelectorAll("[data-list]")) {
		button.addEventListener("click", async () => {
			...
			if (activeList === "project" && macros.project.size === 0) {
				await loadProjectMacros();
			}
			renderList();
		});
	}
...
async function installEditor() {
	...
	try {
		...
		disposeCompletions?.();
		disposeCompletions = registerMacroCompletions(monaco);
		window.addEventListener("pagehide", () => disposeCompletions?.(), { once: true });
		editorReady = true;
		refreshReadOnlyState();
