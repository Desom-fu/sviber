...
// A field control is either the labelable form element itself or a wrapper around one.
function labelableControlElement(element) {
	if (element?.matches?.(LABELABLE_CONTROL_SELECTOR)) {
		return element;
	}
	return element?.querySelector?.(LABELABLE_CONTROL_SELECTOR) ?? null;
}

// Callers may pass either a flat field list or pre-built groups; normalize to groups.
function normalizeFieldGroups(groupsOrFields, options) {
	if (groupsOrFields.length && groupsOrFields[0]?.fields) {
		return groupsOrFields;
	}
	return [{ labelKey: options.groupLabelKey || "panel.properties", fields: groupsOrFields }];
}
...
export class InspectorPanel {
	constructor(options = {}) {
		...
		this.controls = [];
		if (!this.element) {
			throw new Error("InspectorPanel requires an inspector panel element");
		}
	...
	render(groupsOrFields = [], options = {}) {
		this.destroyControls();
		clearElement(this.element);
		const groups = normalizeFieldGroups(groupsOrFields, options);
		...
		for (const group of groups) {
			...
			for (const field of group.fields || []) {
				...
				const input = labelableControlElement(control.element);
				if (input) {
					input.id ||= nextControlId("inspector-field");
					label.htmlFor = input.id;
				}
				const disabled =
					typeof field.disabled === "function" ? field.disabled(options.context) : field.disabled;
				control.setDisabled(Boolean(disabled));
				if (field.tooltipKey || field.tooltip) {
					this.tooltip?.register(label, field.tooltipKey || field.tooltip, {
						raw: Boolean(field.tooltip && !field.tooltipKey),
					});
				}
...
function drawSnappeePreview(canvas, item) {
	...
	if (type.includes("rectangular") || type.includes("parametricMesh")) {
		for (const coordinate of [5, 12, 19]) {
			context.beginPath();
			context.moveTo(coordinate, 3);
			context.lineTo(coordinate, 21);
			context.stroke();
			context.beginPath();
			context.moveTo(3, coordinate);
			context.lineTo(21, coordinate);
			context.stroke();
		}
	} else if (type.includes("radial")) {
		for (const radius of [4, 8]) {
			context.beginPath();
			context.arc(12, 12, radius, 0, Math.PI * 2);
			context.stroke();
		}
		...
		context.beginPath();
		context.arc(12, 12, 9, Math.PI * 0.25, Math.PI * 1.75);
		context.stroke();
	} else {
		context.beginPath();
		context.moveTo(2, 17);
...
export class SnappeesPanel {
	constructor(options = {}) {
		...
		if (!this.element) {
			throw new Error("SnappeesPanel requires a snappees panel element");
		}
		this.unwireTabs = wireSideTabs(this.document);
	...
	select(id, notify = true) {
		...
		this.render();
		if (notify) {
			this.callbacks.onSelect?.(id, this.items.find(item => item.id === id) || null);
		}
	}
	...
	render() {
		for (const element of this.registeredElements) {
			this.tooltip?.unregister(element);
		}
		...
		for (const item of this.items) {
			...
			row.append(
				...
				this.createAction(item, "panel.snappee.duplicate", "duplicate", selected =>
					this.callbacks.onDuplicate?.(selected),
				),
				this.createAction(item, "panel.snappee.delete", "delete", selected =>
...
export class HistoryPanel {
	constructor(options = {}) {
		...
		this.registeredElements = [];
		if (!this.element) {
			throw new Error("HistoryPanel requires a history list element");
		}
		this.unsubscribeLanguage = this.i18n.subscribe(() => this.render());
	}
	...
	seek(pointer) {
		if (pointer < 0 || pointer >= this.items.length || pointer === this.pointer) {
			return;
		}
	...
	labelFor(item) {
		if (item.labelKey) {
			return this.i18n.t(item.labelKey, item.params || {});
		}
