...
export class DialogManager {
	...
	#createFieldEntry(field, values, onFieldChange) {
		...
		// v18 fix: a field that hides its label contributes no cell for the label column, so
		// without `is-full` its control is squeezed into that narrow column. This is what made
		// the "Automatic timing..." parameter groups render as a malformed layout.
		const modifiers = [field.stacked ? "is-stacked" : "", field.hideLabel ? "is-full" : ""].filter(Boolean);
		wrapper.className = ["dialog-field", ...modifiers].join(" ");
