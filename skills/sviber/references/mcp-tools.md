...
- Both sides make pairing visible. The editor toasts "paired" when you allow and "pairing lost" when the server closes. The server logs every tool call on stderr (`sviber-mcp tools/call run_snippet 62ms`) and writes `paired with editor #pid — <chart>` when an editor pairs.
