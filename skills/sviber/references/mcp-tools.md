...
- Backend: one endpoint per editor, not customizable. POSIX uses a Unix socket at `~/.sviber/${pid}.sock`. Windows uses a named pipe `\\.\pipe\sviber-<pid>` plus an empty `<pid>.sock` marker file, because Node's Windows local domain is pipes-only — `listen()`/`connect()` on a filesystem socket path returns `EACCES` there. `SVIBER_DIR_NAME = ".sviber"`. Instances are discovered by listing `<pid>.sock` in that directory; markers of dead editors are swept at discovery time, so a force-killed editor never shows up as a phantom instance.
- Consent is **pairing**, and it is per run and per editor instance. The server announces itself in `~/.sviber/pairing/<id>.json` the moment it starts, under an identity that is unique per server process (`SVIBER_MCP_CLIENT_ID` / `SVIBER_MCP_CLIENT_NAME` pin one), so two servers side by side pair independently instead of overwriting each other. Editors scan that directory at startup and watch it live, so the allow/deny prompt appears when a side starts — not on the first tool call. The prompt shows which client is asking and which sviber instance it would drive.
...
- Both sides make pairing visible. The editor toasts "paired" when you allow and "pairing lost" when the server closes. The server logs every tool call on stderr (`sviber-mcp tools/call run_snippet 62ms`) and writes `paired with editor #pid — <chart>` when an editor pairs. A tool call waits for an on-screen pairing dialog to be answered (the request window is 60s) instead of failing while the user is clicking.
