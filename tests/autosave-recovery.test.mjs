...
test("discarding startup recovery does not mark a manual save", () => {
	...
	assert.equal(manager.recoverable().some(entry => entry.timestamp === timestamp), true);
...
test("applyAutosaveRecovery reopens the full project before overlaying the chart", async () => {
	const App = withHistoryCommands(class {
		constructor() {
			this.calls = [];
			this.difficulties = [
				{ id: "difficulty-0", file: "easy.json", model: ChartModel.createDefault({ metadata: { title: "Easy", difficultyName: "Easy" } }) },
				{ id: "difficulty-1", file: "master.json", model: ChartModel.createDefault({ metadata: { title: "Master", difficultyName: "Master" } }) },
			];
		...
		async clearRuntimeMedia() { this.calls.push("clear"); }
		installProject() { this.calls.push("installStandalone"); }
		async syncMediaFromModel() {}
		refresh() { this.calls.push("refresh"); }
	});
	...
	try {
		const app = new App();
		const recovered = ChartModel.createDefault({ metadata: { title: "Recovered Master", difficultyName: "Master" } });
